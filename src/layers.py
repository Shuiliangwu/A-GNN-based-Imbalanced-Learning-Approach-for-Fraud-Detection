...
# This function choose_step_neighs performs neighborhood sampling for each node in a batch, considering the label-aware scores of the nodes, their 1-hop neighbors, and the minority class nodes. The function returns a list of sampled neighbors and their score differences for each node in the batch.
# The aim is to select neighbors that are more relevant to the central node's label, as well as to perform oversampling for nodes belonging to the minority class. 
def choose_step_neighs(center_scores, center_labels, neigh_scores, neighs_list, minor_scores, minor_list, sample_list, sample_rate):
    ...
    samp_neighs = []
    samp_score_diff = []
    # Iterate through each center_score (label-aware score) in the center_scores list.
    for idx, center_score in enumerate(center_scores):
        # Extract the center_score of the current node (scalar value).
        center_score = center_scores[idx][0]
        # ？ Reshape the label-aware scores of 1-hop neighbors of the current node and store it in neigh_score.
        neigh_score = neigh_scores[idx][:, 0].view(-1, 1)
        # ？ Repeat the center_score to match the shape of neigh_score.
        center_score_neigh = center_score.repeat(neigh_score.size()[0], 1)
        # Extract the neighbor indices of the current node.
        neighs_indices = neighs_list[idx]
        # Extract the number of neighbors to sample for the current node.
        num_sample = sample_list[idx]

        # compute the L1-distance of batch nodes and their neighbors
        # Compute the L1-distance between the label-aware scores of the current node and its 1-hop neighbors, and store it in score_diff_neigh
        score_diff_neigh = torch.abs(center_score_neigh - neigh_score).squeeze()
        # Sort the score differences in ascending order and store the sorted differences and their corresponding indices.
        sorted_score_diff_neigh, sorted_neigh_indices = torch.sort(score_diff_neigh, dim=0, descending=False)
        # Convert the sorted neighbor indices to a Python list.
        selected_neigh_indices = sorted_neigh_indices.tolist()

        # top-p sampling according to distance ranking
        #  If there are more neighbors than required, select the num_sample closest neighbors; otherwise, select all neighbors. Store the selected neighbors in selected_neighs and their score differences in selected_score_diff.
        if len(neigh_scores[idx]) > num_sample + 1:
            ...
            # The code block you provided checks if the variable selected_score_diff is a float. If it is, it converts it to a list with a single element containing the float value. This is done to ensure that selected_score_diff is always a list, even if it only contains one element. This is important because later in the code, selected_score_diff is appended to another list, and if it is not a list, the code will throw an error.
            if isinstance(selected_score_diff, float):
                selected_score_diff = [selected_score_diff]

        # if the label of the current node is 1, it belongs to the minority class, and oversampling should be performed.
        if center_labels[idx] == 1:
            # Calculate the number of oversampled neighbors based on the given sample_rate PICK？
            num_oversample = int(num_sample * sample_rate)
            # ？ Repeat the center_score to match the shape of minor_scores.
            center_score_minor = center_score.repeat(minor_scores.size()[0], 1)
            # ? Compute the L1-distance between the label-aware scores of the current node and the minority nodes, and store it in score_diff_minor
            ...
            # Convert the sorted minority indices to a Python list.
            selected_minor_indices = sorted_minor_indices.tolist()
            # Add the num_oversample closest minority nodes to the selected_neighs list, and their score differences to the selected_score_diff list.
        ...
        # Append the selected neighbors and their score differences for the current node to samp_neighs and samp_score_diff lists, respectively.
        samp_neighs.append(set(selected_neighs))
        samp_score_diff.append(selected_score_diff)

    #  Return the samp_neighs and samp_score_diff lists containing the sampled neighbors and their score differences for each node in the batch.
